...
Every values-carrying template update enqueues one `license_template_sync` job on `pgkit/pgqueue`, **in the same transaction** as the template write — an edit that commits is an edit every license will follow, restart or not. A write that restates the stored values unchanged enqueues too: the job skips each license already in step, so the restatement costs one no-op pass, and it is the repair path for a license that drifted before this rule existed — a Terraform apply of the current configuration re-syncs the tier's customers. A write carrying no values (a rename, a description edit) enqueues nothing. A product can hold far more than the migrate route's 500-organization cap, so the request transaction never re-stamps licenses inline.

The worker pages through the organizations naming the template (100 per job execution, each organization in its own transaction, a continuation job re-enqueued with a cursor) and reads the template **inside each organization's transaction**, never from the job payload. Rapid edits coalesce onto the final state, and two jobs for one template running on two replicas cannot leave a license on the older values: the fresh read under the row lock shrinks the stale window to a commit racing that one transaction, and that commit enqueued its own job, which runs after the lock releases and repairs it. For each license the worker:
