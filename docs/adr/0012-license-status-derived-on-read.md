...
**A limit with no observation on record reads `stale`, with `usage: null` and `last_reported_at: null`.** "No current number exists" is the strongest form of staleness there is, and it needs no further declaration to be knowable.
...
**Deferred:** a per-field expected-reporting-interval declaration, letting `stale` also fire from a report that arrived but has since gone old, was built and then cut from this slice as premature — nothing in Anchor consumes it yet. The `UsageStatus` value and the read's shape both already accommodate it; reintroducing it later is additive.
...
This also means a schema edit is visible immediately, since the schema itself is not part of what is cached here either. Only the license values follow the eviction-on-write discipline; deriving status from them is deliberately as fresh as the rest of the licensing subsystem's writes already are.
...
**Note.** A future SDK facade that caches the license read client-side (per [issue #62](https://github.com/nanostack-dev/anchor/issues/62)'s SDK section) inherits this same shape: it should default to fail-open on `stale`, exactly because `stale` already means "nothing to trust yet," not "something is broken."
