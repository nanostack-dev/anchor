...
| **expected reporting interval** | A per-limit-field declaration of how often its usage is expected to arrive. Used only to derive `stale` from age — Anchor declares what it expects and never pulls a report or checks a schedule itself ([ADR-0012](docs/adr/0012-license-status-derived-on-read.md)). | Not "SLA" or "freshness window". |
