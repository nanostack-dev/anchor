...
- Product-scoped writes that change a catalogued resource must `events.Emit` inside the same `transactor.InTx` as the write. Catalogued resources: organization, membership, workspace, organization API key, product user, organization license. Add the type to `internal/events` first. Clerk ingest is a product write: emit the matching product-user event. Cover the emit with a CT that configures an endpoint (`ProductContext.CaptureEvents`) and asserts a signed Standard Webhooks POST. No endpoint configured is not an error: the worker completes with an empty fan-out.
