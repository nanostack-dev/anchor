...
type ProductRepository interface {
	...
	FindByIDInternal(ctx context.Context, id string, options *toolkit.DBOptions) (*product.Product, error)
	Create(ctx context.Context, prod product.Product, options *toolkit.DBOptions) (
		product.Product, error,
	)
	Update(
	...
	DeleteByID(ctx context.Context, tenantID string, id string, options *toolkit.DBOptions) error
	SearchByTenantID(
	...
	FindAllInternal(ctx context.Context, options *toolkit.DBOptions) ([]product.Product, error)
}
...
func (r *productRepositoryImpl) FindByID(
	...
	return toolkit.QueryOptionalMap[model.Products, product.Product](
		ctx, r.db, stmt,
...
func (r *productRepositoryImpl) Create(
	ctx context.Context, prod product.Product, options *toolkit.DBOptions,
) (product.Product, error) {
	...
	return toolkit.QueryMap[model.Products, product.Product](
...
func (r *productRepositoryImpl) SearchByTenantID(
	...
	slice, err := toolkit.QueryMapSlice(ctx, r.db, query, r.productMapper.ToDomain, options)
	if err != nil {
		r.logger.Error().Err(err).Str(
			"tenantID", tenantID,
...
func (r *productRepositoryImpl) FindAllInternal(
	...
	return toolkit.QueryMapSlice[model.Products, product.Product](
		ctx, r.db, stmt, r.productMapper.ToDomain, options,
	)
}
