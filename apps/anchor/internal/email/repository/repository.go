...
type TemplateRepository interface {
	...
	Create(
		ctx context.Context, template email.Template, options *toolkit.DBOptions,
	) (email.Template, error)
	Update(
		ctx context.Context, tenantID string, template email.Template, options *toolkit.DBOptions,
	) (email.Template, error)
	...
	SaveExamples(
		ctx context.Context, tenantID string, productID string, templateID string, examples []email.TemplateExample, options *toolkit.DBOptions,
	) error
	...
	SetVersionPointers(
		ctx context.Context, tenantID string, templateID string, draftVersionID *string, publishedVersionID *string, options *toolkit.DBOptions,
	) error
	DeleteByID(
		ctx context.Context, tenantID string, productID string, id string, options *toolkit.DBOptions,
	) error
...
type TemplateVersionRepository interface {
	...
	Create(
		ctx context.Context, version email.TemplateVersion, options *toolkit.DBOptions,
	) (email.TemplateVersion, error)
	Update(
		ctx context.Context, version email.TemplateVersion, options *toolkit.DBOptions,
	) (email.TemplateVersion, error)
	...
	UpdateStatus(
		ctx context.Context, id string, status email.TemplateVersionStatus, publishedAt *time.Time, options *toolkit.DBOptions,
	) error
...
type SendRecordRepository interface {
	...
	Create(
		ctx context.Context, tenantID string, productID string, record email.SendRecord, options *toolkit.DBOptions,
	) (email.SendRecord, error)
	...
	UpdateStatusInternal(
		ctx context.Context, id string, status email.SendStatus, lastError *string, sentAt *time.Time, options *toolkit.DBOptions,
	) error
	// UpdateStatus updates the status of a send record, scoped to the tenant.
	UpdateStatus(
		ctx context.Context, tenantID string, id string, status email.SendStatus, lastError *string, sentAt *time.Time, options *toolkit.DBOptions,
	) error
