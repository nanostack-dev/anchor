...
- **Primitives:** shadcn is the source of truth. Local primitives in
  ...
  for convenience.
- **Style reference:** the Echopoint frontend
  (`echopoint/apps/frontend/src/styles/globals.css`) is the reference token
  system. Anchor mirrors it.

> Note: the change proposal prose described a "red primary / warm off-white"
> theme, but the actual Echopoint reference uses a **blue primary**
...
- HSL CSS variables wrapped via `hsl(var(--token))` in `@theme inline`,
  matching Echopoint. Tailwind v4, CSS-first config (no `tailwind.config`).
...
- `components/layout/app-shell.tsx` — local `AppShell` adapter mirroring the
  Echopoint shell composition (skip link, sticky topbar, sidebar inset).
...
| Question | Decision |
| --- | --- |
| Icon library | **Keep Lucide.** Port Echopoint *styling* only; no Phosphor switch. |
...
| `Page` API | **Preserved.** `Page` keeps its `title` / `description` / `breadCrumbs` / `pageInfo` props (now also `actions`); internals restyled to the Echopoint header composition, so the 19 route call-sites are unchanged. |
